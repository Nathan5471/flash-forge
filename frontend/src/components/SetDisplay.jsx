...
export default function SetDisplay({ flashcardSet, isOffline = false }) {
    ...
    return (
        <div className="flex flex-col bg-surface-a1 text-white p-4 rounded-lg">
            <Link to={`${isOffline ? "/downloads" : ""}/set/${flashcardSet._id}`} className="flex flex-col">
                <h2 className="text-xl text-primary-a0 font-bold mb-2">{flashcardSet.title}</h2>
                <p className="mb-4">{flashcardSet.description}</p>
            </Link>    
            <div className="flex flex-row justify-between mt-auto">
                <p className="text-tonal-a4">Created by: <Link to={`${isOffline ? "/downloads" : ""}/user/${flashcardSet.userId._id}`} className='hover:underline'>{flashcardSet.userId.username}</Link></p>
                <p className="text-tonal-a4 font-bold">{flashcardSet.flashCards.length} flashcards</p>
            </div>
            {isOffline && (
