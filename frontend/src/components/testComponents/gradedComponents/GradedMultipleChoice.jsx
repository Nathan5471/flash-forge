        ...
        <div className="flex flex-col items-center justify-center p-4 bg-gray-700 rounded-lg mb-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4 w-full">
                {question.answerChoices.map((answer, index) => (
                    <div
                        key={index}
                        className={`w-full p-2 rounded-lg text-left ${answer === question.answer && 'bg-green-500'} ${(selectedAnswer.selectedAnswer === answer && !selectedAnswer.isCorrect) && 'bg-red-500'} ${(selectedAnswer.selectedAnswer !== answer && answer !== question.answer) && 'bg-gray-800'}`}
                    >{answer}</div>
                ))}
            </div>
        </div>
